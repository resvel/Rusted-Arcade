...
mod tests {
    ...
    #[test]
    fn ps2_allowlist_respects_official_platform_path() {
    ...
    #[test]
    fn ps2_play_override_is_ignored() {
        let expected = if cfg!(all(target_os = "macos", target_arch = "aarch64")) {
            "pcarmsx2"
        } else {
            "pcsx2"
        };
        assert_eq!(resolve_core("PS2", Some("play")), expected);
    }

    #[test]
    fn generic_ps2_scan_core_falls_through_to_platform_default() {
        let expected = if cfg!(all(target_os = "macos", target_arch = "aarch64")) {
            "pcarmsx2"
        } else {
            "pcsx2"
        };
        assert_eq!(resolve_core("PS2", Some("ps2")), expected);
    }
