            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    for controller in controllers.iter() {
                        ...
                        let index = match readings.iter().position(|(other_id, ..)| id == *other_id)
                        {
                            None => {
                                let reading = match WgiGamepad::FromGameController(controller) {
                                    Ok(wgi_gamepad) => Reading::Hybrid(
                                        RawGamepadReading::new(controller).unwrap(),
                                        wgi_gamepad.GetCurrentReading().unwrap(),
                                    ),
                        ...
                        if old_reading.same_sample(new_reading) {
                            continue;
                        }
...
enum Reading {
    ...
    Hybrid(RawGamepadReading, GamepadReading),
}

impl Reading {
    fn same_sample(&self, other: &Self) -> bool {
        match (self, other) {
            (Reading::Raw(left), Reading::Raw(right)) => left.time == right.time,
            (Reading::Gamepad(left), Reading::Gamepad(right)) => left.Timestamp == right.Timestamp,
            (Reading::Hybrid(left_raw, left_gamepad), Reading::Hybrid(right_raw, right_gamepad)) => {
                left_raw.time == right_raw.time && left_gamepad.Timestamp == right_gamepad.Timestamp
            }
            _ => false,
        }
    }
    ...
    fn update(&mut self, controller: &RawGameController) -> windows::core::Result<()> {
        match self {
            ...
            Reading::Hybrid(raw_reading, gamepad_reading) => {
                raw_reading.update(controller)?;
                let gamepad: WgiGamepad = WgiGamepad::FromGameController(controller)?;
                *gamepad_reading = gamepad.GetCurrentReading()?;
            }
    ...
    fn send_events_for_differences(
        ...
        match (old, new) {
            ...
            (Reading::Raw(old), Reading::Raw(new)) => {
                ...
                emit_switch_events(old, new, controller, tx);
            }
            // WGI Gamepad
            ...
            (Reading::Hybrid(old_raw, old_gamepad), Reading::Hybrid(new_raw, new_gamepad)) => {
                emit_switch_events(old_raw, new_raw, controller, tx);

                #[rustfmt::skip]
                let axes = [
                    (new_gamepad.LeftTrigger, old_gamepad.LeftTrigger, nec::AXIS_LT2, 1.0),
                    (new_gamepad.RightTrigger, old_gamepad.RightTrigger, nec::AXIS_RT2, 1.0),
                    (new_gamepad.LeftThumbstickX, old_gamepad.LeftThumbstickX, nec::AXIS_LSTICKX, 1.0),
                    (new_gamepad.LeftThumbstickY, old_gamepad.LeftThumbstickY, nec::AXIS_LSTICKY, -1.0),
                    (new_gamepad.RightThumbstickX, old_gamepad.RightThumbstickX, nec::AXIS_RSTICKX, 1.0),
                    (new_gamepad.RightThumbstickY, old_gamepad.RightThumbstickY, nec::AXIS_RSTICKY, -1.0),
                ];
                for (new, old, code, multiplier) in axes {
                    if new != old {
                        let _ = tx.send(WgiEvent::new(
                ...
                for (current_button, ev_code) in WGI_TO_GILRS_BUTTON_MAP {
                    if (new_gamepad.Buttons & current_button) != (old_gamepad.Buttons & current_button) {
                        let _ = match new_gamepad.Buttons & current_button != GamepadButtons::None {
                            true => tx.send(WgiEvent::new(
                                controller.clone(),
                                EventType::ButtonPressed(ev_code),
                            )),
    ...
    fn is_gamepad(&self) -> bool {
        matches!(self, Reading::Gamepad(_) | Reading::Hybrid(_, _))
    }
}

fn emit_switch_events(
    old: &RawGamepadReading,
    new: &RawGamepadReading,
    controller: &RawGameController,
    tx: &Sender<WgiEvent>,
) {
