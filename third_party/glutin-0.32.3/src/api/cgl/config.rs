...
impl Display {
    ...
    pub(crate) unsafe fn find_configs(
        ...
        let prefer_legacy_profile = env::var_os("ARCADE_MACOS_GL_PROFILE")
            .is_some_and(|value| value.to_string_lossy().eq_ignore_ascii_case("legacy"));

        // By default glutin prefers the newest profile. Some older desktop GL consumers depend on
        // the legacy profile on macOS, so allow a process-level override.
        let profile_order = if prefer_legacy_profile {
            [
