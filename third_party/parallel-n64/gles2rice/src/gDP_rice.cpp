...
void ricegDPLoadTLUT(uint16_t dwCount, uint32_t tileno, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t lrt)
{
   ...
   TileAdditionalInfo *tileinfo = &gRDP.tilesinfo[tileno];
   gDPTile            *tile     = &gDP.tiles[tileno];
   /* TLUT data lives in the upper half of TMEM. Invalid lower-half targets can
    * underflow the palette offset and crash on the palette write below. */
   if (tile->tmem < 0x100)
      return;
   /* Starting location in the palettes */
   uint32_t dwTMEMOffset  = tile->tmem - 0x100;
   ...
   for (uint32_t i=0; i<dwCount && i<0x100 && i + dwTMEMOffset < 0x200; i++)
