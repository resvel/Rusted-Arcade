...
void DLParser_Process(OSTask * pTask)
{
...

    {
        ...
        while( __RSP.PCi >= 0 )
        {
            ...
            if (__RSP.PCi >= MAX_DL_STACK_SIZE)
                break;
            ...
            Gfx *pgfx = (Gfx*)&rdram_u32[pc >> 2];
            __RSP.PC[__RSP.PCi] += 8;
            const uint32_t opcode = (pgfx->words.w0 >> 24) & 0xFF;
            if (currentUcodeMap[opcode] == NULL)
                break;
            currentUcodeMap[opcode](pgfx);
...
void RDP_DLParser_Process(void)
{
    ...
    while (__RSP.PCi >= 0 && __RSP.PCi < MAX_DL_STACK_SIZE && __RSP.PC[__RSP.PCi] < end)
    {
        const uint32_t pc = __RSP.PC[__RSP.PCi];
        if (pc > g_dwRamSize - 8)
            break;
