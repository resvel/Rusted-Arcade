...
static void trace_boot_pi_dma(struct pi_controller *pi,
      const uint8_t *rom, uint32_t rom_address,
      const uint8_t *dram, uint32_t dram_address,
      uint32_t length)
{
   unsigned int j;

   if (getenv("LIBRETRO_TRACE_PI_BOOT_DMA") == NULL)
      return;

   if (pi->regs[PI_CART_ADDR_REG] != UINT32_C(0x10001000))
      return;

   if (length < 16)
      return;

   fprintf(stderr, "pi_boot_dma cart=%#x dram=%#x len=%#x src=", pi->regs[PI_CART_ADDR_REG], pi->regs[PI_DRAM_ADDR_REG], length);
   for (j = 0; j < 16; ++j)
      fprintf(stderr, "%02x", rom[(rom_address + j) ^ S8]);
   fprintf(stderr, " dst=");
   for (j = 0; j < 16; ++j)
      fprintf(stderr, "%02x", dram[(dram_address + j) ^ S8]);
   fprintf(stderr, "\n");
}
...
static void dma_pi_write(struct pi_controller *pi)
{
   ...
   trace_boot_pi_dma(pi, rom, rom_address, dram, dram_address, length);
