...
static unsigned int pure_interp_trace_boot_count;
static unsigned int pure_interp_dumped_boot_block;

static void InterpretOpcode(void);

static int should_trace_pure_boot(void)
{
   return getenv("LIBRETRO_TRACE_PURE_BOOT") != NULL;
}

static int should_trace_pure_boot_compare(void)
{
   return getenv("LIBRETRO_TRACE_PURE_BOOT_COMPARE") != NULL;
}

static void trace_pure_boot_state(uint32_t op)
{
   uint32_t min_addr = UINT32_C(0x80000180);
   uint32_t max_addr = UINT32_C(0x800001d0);

   if (!should_trace_pure_boot())
      return;

   if (pure_interp_trace_boot_count >= 256)
      return;

   if (should_trace_pure_boot_compare())
      min_addr = UINT32_C(0x800001a0);

   if (PC->addr < min_addr || PC->addr > max_addr)
      return;

   fprintf(stderr,
         "pure_boot pc=%#x op=%#x v0=%#llx v1=%#llx a0=%#llx a1=%#llx a2=%#llx a3=%#llx "
         "t0=%#llx t1=%#llx t2=%#llx t3=%#llx t4=%#llx t5=%#llx t6=%#llx t7=%#llx "
         "s0=%#llx s1=%#llx s2=%#llx s3=%#llx s4=%#llx s5=%#llx s6=%#llx s7=%#llx "
         "sp=%#llx ra=%#llx count=%#x next_interrupt=%#x\n",
         PC->addr, op,
         (unsigned long long) reg[2], (unsigned long long) reg[3],
         (unsigned long long) reg[4], (unsigned long long) reg[5],
         (unsigned long long) reg[6], (unsigned long long) reg[7],
         (unsigned long long) reg[8], (unsigned long long) reg[9],
         (unsigned long long) reg[10], (unsigned long long) reg[11],
         (unsigned long long) reg[12], (unsigned long long) reg[13],
         (unsigned long long) reg[14], (unsigned long long) reg[15],
         (unsigned long long) reg[16], (unsigned long long) reg[17],
         (unsigned long long) reg[18], (unsigned long long) reg[19],
         (unsigned long long) reg[20], (unsigned long long) reg[21],
         (unsigned long long) reg[22], (unsigned long long) reg[23],
         (unsigned long long) reg[29], (unsigned long long) reg[31],
         g_cp0_regs[CP0_COUNT_REG], next_interrupt);
   pure_interp_trace_boot_count++;
}

static void dump_pure_boot_block(void)
{
   uint32_t addr;

   if (pure_interp_dumped_boot_block)
      return;

   if (getenv("LIBRETRO_DUMP_PURE_BOOT_BLOCK") == NULL)
      return;

   if (PC->addr < UINT32_C(0x80000120) || PC->addr > UINT32_C(0x800001d0))
      return;

   pure_interp_dumped_boot_block = 1;
   for (addr = UINT32_C(0x80000120); addr <= UINT32_C(0x800001d0); addr += 16)
   {
      fprintf(stderr,
            "pure_boot_block addr=%#x words=%#x,%#x,%#x,%#x\n",
            addr,
            *fast_mem_access(addr),
            *fast_mem_access(addr + 4),
            *fast_mem_access(addr + 8),
            *fast_mem_access(addr + 12));
   }
}
...
void InterpretOpcode(void)
{
	...
	dump_pure_boot_block();
	trace_pure_boot_state(op);
	switch ((op >> 26) & 0x3F) {
...
{
   stop = 0;
   pure_interp_trace_boot_count = 0;
   pure_interp_dumped_boot_block = 0;
   PC = &interp_PC;
