...
static const char* interrupt_event_name(int type)
{
   switch (type)
   {
      case SPECIAL_INT: return "SPECIAL_INT";
      case VI_INT:      return "VI_INT";
      case COMPARE_INT: return "COMPARE_INT";
      case CHECK_INT:   return "CHECK_INT";
      case SI_INT:      return "SI_INT";
      case PI_INT:      return "PI_INT";
      case AI_INT:      return "AI_INT";
      case SP_INT:      return "SP_INT";
      case DP_INT:      return "DP_INT";
      case HW2_INT:     return "HW2_INT";
      case NMI_INT:     return "NMI_INT";
      case CART_INT:    return "CART_INT";
      default:          return "UNKNOWN_INT";
   }
}

static const char* cp0_exccode_name(uint32_t cause)
{
   switch (cause & CP0_CAUSE_EXCCODE_MASK)
   {
      case CP0_CAUSE_EXCCODE_INT:   return "INT";
      case CP0_CAUSE_EXCCODE_MOD:   return "MOD";
      case CP0_CAUSE_EXCCODE_TLBL:  return "TLBL";
      case CP0_CAUSE_EXCCODE_TLBS:  return "TLBS";
      case CP0_CAUSE_EXCCODE_ADEL:  return "ADEL";
      case CP0_CAUSE_EXCCODE_ADES:  return "ADES";
      case CP0_CAUSE_EXCCODE_IBE:   return "IBE";
      case CP0_CAUSE_EXCCODE_DBE:   return "DBE";
      case CP0_CAUSE_EXCCODE_SYS:   return "SYS";
      case CP0_CAUSE_EXCCODE_BP:    return "BP";
      case CP0_CAUSE_EXCCODE_RI:    return "RI";
      case CP0_CAUSE_EXCCODE_CPU:   return "CPU";
      case CP0_CAUSE_EXCCODE_OV:    return "OV";
      case CP0_CAUSE_EXCCODE_TR:    return "TR";
      case CP0_CAUSE_EXCCODE_FPE:   return "FPE";
      case CP0_CAUSE_EXCCODE_WATCH: return "WATCH";
...
{
   static unsigned trace_exception_count = 0;
   if (getenv("LIBRETRO_TRACE_EXCEPTIONS") && trace_exception_count < 256)
   {
      uint32_t pc = (PC != NULL) ? PC->addr : 0;
#ifdef NEW_DYNAREC
      if (r4300emu == CORE_DYNAREC)
         pc = pcaddr;
#endif
      fprintf(stderr,
              "exception_general pc=%#x cause=%#x(%s) status=%#x epc=%#x badvaddr=%#x next=%s\n",
              pc,
              g_cp0_regs[CP0_CAUSE_REG],
              cp0_exccode_name(g_cp0_regs[CP0_CAUSE_REG]),
              g_cp0_regs[CP0_STATUS_REG],
              g_cp0_regs[CP0_EPC_REG],
              g_cp0_regs[CP0_BADVADDR_REG],
              q.first ? interrupt_event_name(q.first->data.type) : "none");
...
{
   static unsigned trace_interrupt_count = 0;
   ...
   if (getenv("LIBRETRO_TRACE_INTERRUPTS") && trace_interrupt_count < 256 && q.first != NULL)
   {
      fprintf(stderr,
              "gen_interrupt event=%s count=%u cp0_count=%#x status=%#x cause=%#x next_interrupt=%#x\n",
              interrupt_event_name(q.first->data.type),
              q.first->data.count,
              g_cp0_regs[CP0_COUNT_REG],
              g_cp0_regs[CP0_STATUS_REG],
              g_cp0_regs[CP0_CAUSE_REG],
              next_interrupt);
      trace_interrupt_count++;
   }
