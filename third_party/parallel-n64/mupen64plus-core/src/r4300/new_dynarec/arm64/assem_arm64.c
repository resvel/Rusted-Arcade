...
#define DIRTY_STUB_CALL_SEARCH_WORDS 12

static int decode_direct_bl_target(const u_int *ptr, uintptr_t *target, int *insn_count)
{
  if((ptr[0]&0xfc000000)!=0x94000000)
    return 0;

  *target=(uintptr_t)((intptr_t)ptr+(((signed int)(ptr[0]<<6)>>6)<<2));
  *insn_count=1;
  return 1;
}

#if defined(__APPLE__)
static int decode_movimm64_x16_call_target(const u_int *ptr, uintptr_t *target, int *insn_count)
{
  if((ptr[0]&0xffe0001f)!=0xd2800010) return 0;
  if((ptr[1]&0xffe0001f)!=0xf2a00010) return 0;
  if((ptr[2]&0xffe0001f)!=0xf2c00010) return 0;
  if((ptr[3]&0xffe0001f)!=0xf2e00010) return 0;
  if(ptr[4]!=0xd63f0200) return 0;

  *target=
      ((uintptr_t)((ptr[0]>>5)&0xffff)      ) |
      ((uintptr_t)((ptr[1]>>5)&0xffff) << 16) |
      ((uintptr_t)((ptr[2]>>5)&0xffff) << 32) |
      ((uintptr_t)((ptr[3]>>5)&0xffff) << 48);
  *insn_count=5;
  return 1;
}
#endif

static int decode_call_target(const u_int *ptr, uintptr_t *target, int *insn_count)
{
  if(decode_direct_bl_target(ptr,target,insn_count))
    return 1;
#if defined(__APPLE__)
  if(decode_movimm64_x16_call_target(ptr,target,insn_count))
    return 1;
#endif
  return 0;
}

static u_int *find_call_instruction(u_int *ptr, int max_words, uintptr_t *target, int *insn_count)
{
  int i;
  for(i=0;i<max_words;i++) {
    if(decode_call_target(ptr+i,target,insn_count))
      return ptr+i;
  }
  return NULL;
}
...
static uintptr_t get_clean_addr(intptr_t addr)
{
  ...
  int call_words=0;
  ptr=find_call_instruction((u_int *)ptr,DIRTY_STUB_CALL_SEARCH_WORDS,&verifier,&call_words);
  assert(ptr!=NULL);
  ptr+=call_words;
  if((*ptr&0xfc000000)==0x14000000) { //b
...
static int verify_dirty(void *addr)
{
  ...
  uintptr_t verifier=0;
  int call_words=0;
  ptr=find_call_instruction(ptr,6,&verifier,&call_words);
  assert(ptr!=NULL);
...
static int is_direct_jmp_range(uintptr_t addr)
{
  if(addr<4) return 1;
  {
    intptr_t offset=addr-(intptr_t)out;
    return offset>=-134217728&&offset<134217728;
  }
}
...
static void trace_far_branch(const char *kind, uintptr_t from, uintptr_t target)
{
  if(getenv("LIBRETRO_TRACE_DYNAREC_FAR_BRANCHES")==NULL)
    return;

  DebugMessage(M64MSG_INFO,
      "dynarec far %s: from=%p target=%p delta=%lld",
      kind, (void *)from, (void *)target,
      (long long)((intptr_t)target-(intptr_t)from));
}
...
static void emit_movimm64_addr(uintptr_t imm,u_int rt)
{
  emit_movz64((u_int)(imm&0xffff),rt);
  emit_movk64_lsl16((u_int)((imm>>16)&0xffff),rt);
  emit_movk64_lsl32((u_int)((imm>>32)&0xffff),rt);
  emit_movk64_lsl48((u_int)((imm>>48)&0xffff),rt);
}
...
{
#if defined(__APPLE__)
  if(!is_direct_jmp_range((uintptr_t)a)) {
    trace_far_branch("call",(uintptr_t)out,(uintptr_t)a);
    assem_debug("load %x; blr x16",a);
    emit_movimm64_addr((uintptr_t)a,16);
    output_w32(0xd63f0200);
    return;
  }
#endif
...
{
#if defined(__APPLE__)
  if(a!=0&&!is_direct_jmp_range((uintptr_t)a)) {
    trace_far_branch("jump",(uintptr_t)out,(uintptr_t)a);
    assem_debug("load %x; br x16",a);
    emit_movimm64_addr((uintptr_t)a,16);
    output_w32(0xd61f0000|16<<5);
    return;
  }
#endif
