...
static int new_dynarec_available;

#if defined(__APPLE__) && NEW_DYNAREC >= NEW_DYNAREC_ARM
static void dynarec_begin_code_write(void)
{
  if (pthread_jit_write_protect_supported_np())
    pthread_jit_write_protect_np(0);
}

static void dynarec_end_code_write(void)
{
  if (pthread_jit_write_protect_supported_np())
...
#if defined(__APPLE__) && NEW_DYNAREC >= NEW_DYNAREC_ARM
static void *dynarec_alloc_jit_region(size_t size)
{
...
static void trace_dynarec_layout(void)
{
  if (getenv("LIBRETRO_TRACE_DYNAREC_LAYOUT") == NULL)
    return;

  DebugMessage(M64MSG_INFO,
      "dynarec layout: base=%p readmem_dword=%p PC=%p fake_pc=%p mini_ht=%p invalid_code=%p",
      base_addr, &readmem_dword, &PC, &fake_pc, &mini_ht, invalid_code);
  DebugMessage(M64MSG_INFO,
      "dynarec deltas: PC-base=%lld fake_pc-base=%lld readmem_dword-base=%lld mini_ht-base=%lld invalid_code-base=%lld",
      (long long)((intptr_t)&PC - (intptr_t)base_addr),
      (long long)((intptr_t)&fake_pc - (intptr_t)base_addr),
      (long long)((intptr_t)&readmem_dword - (intptr_t)base_addr),
      (long long)((intptr_t)&mini_ht - (intptr_t)base_addr),
      (long long)((intptr_t)invalid_code - (intptr_t)base_addr));
}

static void trace_suspicious_lookup(const char *origin, u_int vaddr, void *addr)
{
  if (getenv("LIBRETRO_TRACE_DYNAREC_LOOKUPS") == NULL)
    return;
  if (addr == NULL || base_addr == NULL)
    return;

  uintptr_t target = (uintptr_t) addr;
  uintptr_t base = (uintptr_t) base_addr;
  uintptr_t end = base + ((uintptr_t)1 << TARGET_SIZE_2);
  if (target >= base && target < end)
    return;

  DebugMessage(M64MSG_WARNING,
      "dynarec suspicious lookup: %s vaddr=%08x addr=%p base=%p end=%p",
      origin, vaddr, addr, base_addr, (void *) end);
}
...
void *get_addr(u_int vaddr)
{
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr match %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(intptr_t)head->addr);
      ...
      #endif
      trace_suspicious_lookup("get_addr.jump_in", vaddr, head->addr);
      return head->addr;
    }
    head=head->next;
  }
    ...
    if(head->vaddr==vaddr&&head->reg32==0) {
      ...
      if((((uintptr_t)head->addr-(uintptr_t)out)<<(32-TARGET_SIZE_2))>0x60000000+(MAX_OUTPUT_BLOCK_SIZE<<(32-TARGET_SIZE_2))) {
        if(verify_dirty(head->addr)) {
          ...
          print_debug_info(vaddr);
          #endif
          trace_suspicious_lookup("get_addr.jump_dirty", vaddr, head->addr);
          return head->addr;
        }
      }
    }
...
void *get_addr_ht(u_int vaddr)
{
  ...
  if(ht_bin[0]==vaddr){
    ...
    print_debug_info(vaddr);
    #endif
    trace_suspicious_lookup("get_addr_ht.hash0", vaddr, (void *)ht_bin[1]);
  ...
  if(ht_bin[2]==vaddr){
    ...
    print_debug_info(vaddr);
    #endif
    trace_suspicious_lookup("get_addr_ht.hash1", vaddr, (void *)ht_bin[3]);
...
void *get_addr_32(u_int vaddr,u_int flags)
{
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&(head->reg32&flags)==0) {
      ...
      print_debug_info(vaddr);
      #endif
      trace_suspicious_lookup("get_addr_32.jump_in", vaddr, head->addr);
      return head->addr;
    }
    head=head->next;
  }
    ...
    if(head->vaddr==vaddr&&(head->reg32&flags)==0) {
      ...
      if((((uintptr_t)head->addr-(uintptr_t)out)<<(32-TARGET_SIZE_2))>0x60000000+(MAX_OUTPUT_BLOCK_SIZE<<(32-TARGET_SIZE_2))) {
        if(verify_dirty(head->addr)) {
          ...
          print_debug_info(vaddr);
          #endif
          trace_suspicious_lookup("get_addr_32.jump_dirty", vaddr, head->addr);
          return head->addr;
        }
      }
    }
...
void new_dynarec_init(void)
{
  ...
  new_dynarec_available = 0;
#if NEW_DYNAREC >= NEW_DYNAREC_ARM
#if defined(__APPLE__)
  {
    base_addr = dynarec_alloc_jit_region(1<<TARGET_SIZE_2);
    if (base_addr == NULL) {
      DebugMessage(M64MSG_ERROR,
          "mmap() failed to allocate MAP_JIT region: %s",
          strerror(errno));
    }
  }
#else
  ...
  new_dynarec_available = 1;
  trace_dynarec_layout();
...
int new_dynarec_is_available(void)
{
  return new_dynarec_available;
}
