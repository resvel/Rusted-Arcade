...
static int should_trace_rsp(void)
{
    return getenv("LIBRETRO_TRACE_RSP") != NULL;
}
...
{
   static unsigned trace_rsp_write_count = 0;
   unsigned l, length, count, skip;
   struct rsp_core* sp = (struct rsp_core*)opaque;
   uint32_t reg        = RSP_REG(address);

   if (should_trace_rsp() && trace_rsp_write_count < 256)
   {
      fprintf(stderr,
              "write_rsp_regs reg=%u value=%#x mask=%#x before=%#x pc=%#x\n",
              reg, value, mask,
              (reg < SP_REGS_COUNT) ? sp->regs[reg] : 0u,
              sp->regs2[SP_PC_REG]);
      trace_rsp_write_count++;
   }
...
{
    static unsigned trace_rsp_write2_count = 0;
    struct rsp_core* sp = (struct rsp_core*)opaque;
    uint32_t reg        = RSP_REG2(address);

    if (should_trace_rsp() && trace_rsp_write2_count < 256)
    {
        fprintf(stderr,
                "write_rsp_regs2 reg=%u value=%#x mask=%#x before=%#x\n",
                reg, value, mask,
                (reg < SP_REGS2_COUNT) ? sp->regs2[reg] : 0u);
        trace_rsp_write2_count++;
    }
...
{
    static unsigned trace_rsp_task_count = 0;
    uint32_t save_pc = sp->regs2[SP_PC_REG] & ~0xfff;

    if (should_trace_rsp() && trace_rsp_task_count < 64)
    {
        fprintf(stderr,
                "do_SP_Task task=%#x status=%#x sp_pc=%#x mi_intr=%#x\n",
                sp->mem[0xfc0/4], sp->regs[SP_STATUS_REG],
                sp->regs2[SP_PC_REG], sp->r4300->mi.regs[MI_INTR_REG]);
        trace_rsp_task_count++;
    }
