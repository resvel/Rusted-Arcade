...
void pifbootrom_hle_execute(struct device *dev)
{
    ...
    if (getenv("LIBRETRO_TRACE_BOOT"))
    {
      DebugMessage(M64MSG_INFO,
            "PIF boot rom_type=%u tv_type=%u cic_seed=%#x sp_status=%#x pi_status=%#x vi_intr=%#x",
            rom_type, tv_type, dev->si.pif.cic.seed,
            dev->sp.regs[SP_STATUS_REG], dev->pi.regs[PI_STATUS_REG],
            dev->vi.regs[VI_V_INTR_REG]);
    }
