...
static bool vi_process_full(void)
{
    ...
    static unsigned trace_full_failures = 0;
    ...
    if (isblank && prevwasblank) {
        if (getenv("LIBRETRO_TRACE_VI") && trace_full_failures < 64) {
            fprintf(stderr,
                    "vi_process_full invalid=isblank_prevblank type=%u hres=%d h_start=%d vres=%d v_start=%d fb=%#x v_sync=%d\n",
                    ctrl.type, hres, h_start, vres, v_start, frame_buffer, v_sync);
            trace_full_failures++;
        }
        return false;
    }
    ...
    if (!validh) {
        if (getenv("LIBRETRO_TRACE_VI") && trace_full_failures < 64) {
            fprintf(stderr,
                    "vi_process_full invalid=validh hres=%d h_start=%d type=%u fb=%#x vactivelines=%d\n",
                    hres, h_start, ctrl.type, frame_buffer, vactivelines);
            trace_full_failures++;
        }
        return false;
    }
...
{
    static unsigned trace_fast_failures = 0;
    ...
    if (hres_raw <= 0 || vres_raw <= 0) {
        if (getenv("LIBRETRO_TRACE_VI") && trace_fast_failures < 64) {
            fprintf(stderr,
                    "vi_process_fast invalid=size hres_raw=%d vres_raw=%d hres=%d vres=%d x_add=%u y_add=%u fb=%#x\n",
                    hres_raw, vres_raw, hres, vres, x_add, y_add, frame_buffer);
            trace_fast_failures++;
        }
        return false;
    }
    ...
    if (!(ctrl.type & 2)) {
        if (getenv("LIBRETRO_TRACE_VI") && trace_fast_failures < 64) {
            fprintf(stderr,
                    "vi_process_fast invalid=type type=%u hres_raw=%d vres_raw=%d fb=%#x\n",
                    ctrl.type, hres_raw, vres_raw, frame_buffer);
            trace_fast_failures++;
        }
        return false;
    }
...
{
    static unsigned trace_update_count = 0;
    ...
    if (getenv("LIBRETRO_TRACE_VI") && trace_update_count < 64) {
        fprintf(stderr,
                "vi_update mode=%u fb=%#x type_pre=%u h_start=%d h_end=%d hres=%d v_start=%d v_end=%d vres=%d x_add=%u y_add=%u v_sync=%d vi_width=%u current_line=%d\n",
                config.vi.mode, frame_buffer, *vi_reg_ptr[VI_STATUS] & 3, h_start, h_end, hres,
                v_start, v_end, vres, x_add, y_add, v_sync, vi_width_low, v_current_line);
        trace_update_count++;
    }

    // cancel if the frame buffer contains no valid address
    if (!frame_buffer) {
        if (getenv("LIBRETRO_TRACE_VI") && trace_update_count < 96) {
            fprintf(stderr, "vi_update invalid=frame_buffer_zero\n");
            trace_update_count++;
        }
        vdac_sync(true);
        return;
    ...
    if (getenv("LIBRETRO_TRACE_VI") && trace_update_count < 96) {
        fprintf(stderr,
                "vi_update result valid=%d type=%u hres=%d vres=%d h_start=%d v_start=%d vactivelines=%d serrate=%u\n",
                valid ? 1 : 0, ctrl.type, hres, vres, h_start, v_start, vactivelines, ctrl.serrate);
        trace_update_count++;
    }
