...
static unsigned gl_trace_draw_calls = 0;
static unsigned gl_trace_draw_vertices = 0;
static unsigned gl_trace_clear_calls = 0;

static int gl_trace_enabled(void)
{
   return getenv("LIBRETRO_TRACE_DRAWS") != NULL;
}

static void gl_trace_log_frame(void)
{
   if (!gl_trace_enabled())
      return;
...
#endif
      fprintf(stderr,
            "gles2n64 frame draws=%u vertices=%u clears=%u program=%d array_buffer=%d element_array_buffer=%d render_state=%u screen_update=%d must_render=%d\n",
            gl_trace_draw_calls,
            gl_trace_draw_vertices,
            gl_trace_clear_calls,
            program,
            array_buffer,
            element_array_buffer,
            OGL.renderState,
            OGL.screenUpdate ? 1 : 0,
            OGL.mustRenderDlist);
   }

   gl_trace_draw_calls = 0;
   gl_trace_draw_vertices = 0;
   gl_trace_clear_calls = 0;
}
...
void OGL_DrawLLETriangle(uint32_t _numVtx)
{
   ...
   gl_trace_draw_calls++;
   gl_trace_draw_vertices += _numVtx;
	OGL.triangles.num = 0;
...
void OGL_DrawTriangles(void)
{
   ...
   gl_trace_draw_calls++;
   gl_trace_draw_vertices += OGL.triangles.num;
   OGL.triangles.num = 0;
}
...
void OGL_SwapBuffers(void)
{
   ...
   gl_trace_log_frame();
   if (getenv("LIBRETRO_TRACE_SWAPS"))
      fprintf(stderr, "gles2n64 OGL_SwapBuffers renderCallback=%d\n", renderCallback ? 1 : 0);
